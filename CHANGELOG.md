...
- A run left open when the device rebooted was silently ended for good if its source was not ready yet — at boot the Bluetooth adapter routinely is not — instead of staying resumable
...
- The GPS device is chosen from one Settings row that names the current source and opens a picker when tapped, replacing the dropdown field. Settings are grouped under Recording, Diagnostics and About
...
- While recording in such a window Record now uses the whole height: the live values sit in a full-height panel, centred as one block, with the status and controls centred beside them, instead of everything sitting in the top third with the rest of the screen empty
